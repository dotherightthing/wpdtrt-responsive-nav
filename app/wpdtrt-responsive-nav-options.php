...
if ( !function_exists( 'wpdtrt_responsive_nav_options_page' ) ) {
   ...
   *       a. The user's selection: 'wpdtrt_responsive_nav_datatype',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_responsive_nav_data'
  ...
  function wpdtrt_responsive_nav_options_page() {
    ...
    if ( isset( $_POST['wpdtrt_responsive_nav_form_submitted'] ) ) {
      ...
      if ( $hidden_field === 'Y' ) {
        ...
        $wpdtrt_responsive_nav_datatype = esc_html( $_POST['wpdtrt_responsive_nav_datatype'] );

        // 2. send user preferences to API to get results
        $wpdtrt_responsive_nav_data = wpdtrt_responsive_nav_get_data(
          $wpdtrt_responsive_nav_datatype
        );

        // 3a. store user preferences in options object
        $wpdtrt_responsive_nav_options['wpdtrt_responsive_nav_datatype'] = $wpdtrt_responsive_nav_datatype;

        // 3b. store API results in options object
        $wpdtrt_responsive_nav_options['wpdtrt_responsive_nav_data'] = $wpdtrt_responsive_nav_data;
